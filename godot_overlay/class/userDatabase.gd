	...
	if ResourceLoader.exists(db_path):
		...
		if file:
			...
			if json is Dictionary:
				userData = json
				print("[UserDB] 데이터베이스 로드 완료: %d명의 사용자" % userData.size())
			else:
				print("[UserDB] JSON 파싱 중 오류가 발생했습니다.")
				userData = {}
	else:
		print("[UserDB] 데이터베이스 파일이 없습니다. 새로운 파일을 생성합니다.")
		userData = {}
...
func save_database() -> void:
	...
	if file:
		...
		print("[UserDB] 데이터베이스에 성공적으로 저장되었습니다.")
	else:
		print("[UserDB] 데이터베이스에 저장하지 못했습니다.")
...
func add_user(CID: String, nickname: String, avatar: String) -> void:
	if not userData.has(CID):
		...
		save_database()
		print("[UserDB] 새로운 사용자 추가됨: %s (%s)" % [nickname, CID])
	else:
		# 이미 존재하면 닉네임만 업데이트
...
func update_user_avatar(CID: String, avatar_name: String) -> void:
	if userData.has(CID):
		userData[CID]["AVATAR"] = avatar_name
		userData[CID]["LAST_SEEN"] = Time.get_ticks_msec() / 1000.0
		save_database()
